...
(function() {
   ...
   * @constructor
   * @param {array}
   *            optional plain array to populate a power array
   */
  function PowerArray(array) {
    var load = array || [], self = this;
  ...
  /**
   * addAndSort inserts a value and sorts the array after insertion to keep
  ...
  /**
   * forEach: improved version of the Array.prototype.forEach method, utilises
   * for loop
   * 
   * @param {function}
   *            processing function
  ...
  /**
   * map: improved version of map re-utilizing PowerArray.forEach did not make
   * performance differences, so for-loop was used to maintain the 5x factor
   * 
   * @param {function}
   *            mapping function
  ...
  PowerArray.prototype.map = function(fun) {
    var results = new PowerArray(), len = this.length, i;
    for (i = 0; i < len; i += 1) {
  ...
  PowerArray.prototype.includes = function(elem) {
    var i = 0, len = this.length, found = false;
    for (i; i < len; i += 1) {
      if (this[i] === elem) {
        found = true;
        break;
  ...
  PowerArray.prototype.slice = function() {
    var args = Array.prototype.slice.call(arguments), results = Array.prototype.slice.apply(this, args);
   ...
   *            end
   * @param {object}
   *            varargs, elements to add
   * @returns {object} a PowerArray
   */
  PowerArray.prototype.splice = function() {
    var args = Array.prototype.slice.call(arguments), results = Array.prototype.splice.apply(this, args);
    return new PowerArray(results);
  };
  ...
  console.debug("Using powerarray");
}).call(this);